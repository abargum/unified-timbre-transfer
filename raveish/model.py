...
from raveish.augmentations import ComposeTransforms, AddNoise, PitchAug, SloppyPEQ

import raveish.core
from raveish.core import extract_pitch, a_weighted_loudness, z_score_normalization, warmup_lambda
from raveish.core import bins_to_frequency, entropy
from . import blocks
from .utils import perceptive 

mds_dims = 2
fullTimbreData = perceptive.get_perceptual_centroids(mds_dims, timbre_path='raveish/utils/timbre.npy')
...
@gin.configurable
class UNIFIED_TT(pl.LightningModule):
    ...
    def __init__(
        ...
        assert weights is not None, "UNIFIED_TT model requires either weights or loss_weights (depreciated) keyword"
        ...
        if self.load_pitch_enc:
            self.pitch_encoder.load_state_dict(torch.load(f"raveish/utils/noncaus2048_mb6.pth", weights_only=True))
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.valid_signal_crop and self.receptive_field.sum():
            x_multiband = raveish.core.valid_signal_crop(
                x_multiband,
                *self.receptive_field,
            )
            y_multiband = raveish.core.valid_signal_crop(
                y_multiband,
                *self.receptive_field,
            )
        p.tick('crop')
