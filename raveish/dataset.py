...
class LazyAudioDataset(data.Dataset):
    ...
    def __init__(self,
        ...
        # Group sample indices by instrument type for easy pairing
        self.instrument_to_indices = defaultdict(list)
        for idx, key in enumerate(self.keys):
            with self.env.begin() as txn:
                ae = AudioExample.FromString(txn.get(key))
            instrument = ae.metadata['path'].split('/')[-2]
            self.instrument_to_indices[instrument].append(idx)
    ...
    def is_silent(self, audio):
        """Check if the audio is silent based on a threshold."""
        return np.mean(np.abs(audio)) < self.silence_threshold

    def __getitem__(self, index):
        # Retrieve primary sample
        ...
        instrument = ae.metadata['path'].split('/')[-3]
...
def random_angle(min_f=20, max_f=8000, sr=24000):
    min_f = np.log(min_f)
    max_f = np.log(max_f)
    rand = np.exp(random.random() * (max_f - min_f) + min_f)
